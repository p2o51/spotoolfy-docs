...
显示/隐藏 Seek 控制层。单击封面会切换一个半透明遮罩层，包含三个按钮：
- **-10s（左）**：快退 10 秒
- **信息（中）**：打开 AI 歌曲分析页面
- **+10s（右）**：快进 10 秒
...
跳转到 Spotify 应用。长按封面会尝试：
1. 打开当前播放上下文（专辑/播放列表）在 Spotify 中
2. 若无上下文，打开当前曲目在 Spotify 中
3. 若都失败，直接打开 Spotify 应用
...
点击封面控制层的「信息」按钮后会打开歌曲分析页面，展示 Gemini AI 生成的歌曲分析。
...
| 作曲人 | 曲作者 |
| 制作人 | 音乐制作人 |
| 歌曲解析 | AI 生成的专业音乐评论和背景分析（2-4 句）|

#### 特性
- 使用 Google Search Grounding 确保信息准确
- 结果会本地缓存，避免重复请求
- 支持重新生成（右上角刷新按钮）
- 内容可复制到剪贴板

#### 追问功能
页面底部有「Ask Gemini」浮动按钮，可以：
- 对歌曲提出追问
- 基于已有分析上下文进行深入探讨
- 提供对话式交互体验
...
| 服务 | 用途 |
|------|------|
| SongInfoService | 生成单曲 AI 分析 |
| InsightsService | 基于听歌历史生成音乐个性分析 |
